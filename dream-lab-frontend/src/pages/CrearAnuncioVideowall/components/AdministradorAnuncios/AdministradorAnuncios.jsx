...
function AdministradorAnuncios(props) {
    ...
    const [mostrarCambiosAplicados, setMostrarCambiosAplicados] =
        useState(false);
    ...
    const eliminarAnuncio = (anuncioId) => {
        const newData = data.filter(
            (anuncio) => anuncio.firebaseId !== anuncioId
        );
        setData(newData);
    };
    ...
    const enviarCambios = async () => {
        try {
            await Promise.all(
                data.map(async (anuncio, index) => {
                    await fetch(
                        `https://updateanuncio2-j5zt2ysdwq-uc.a.run.app?id=${anuncio.firebaseId}`,
                        {
                            method: "PUT",
                            headers: {
            ...
            <div className="contenedor-admin-anuncios">
                <h1 className="sub-admin-anuncios">
                    {mostrarCambiosAplicados
                        ...
                        : cambiosPendientes
                        ? `Envío de cambios en ${tiempoRestante} segundos`
                ...
                {mostrarModal && (
                    <ModalEliminacionAnuncio
                        ...
                        eliminarAnuncio={eliminarAnuncio} // Pasa la función para eliminar un anuncio
                    />
                )}
            </div>
        </DndProvider>
