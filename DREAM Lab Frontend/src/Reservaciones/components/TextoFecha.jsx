...
import { getFromSessionStorage, existsInSessionStorage } from "../../Global/Storage";
...
const TextoFecha = ( {update} ) => {
    ...
    const getHoraInicioTexto = (horaInicio) => {
        return horaInicio < 12 ? `${horaInicio} AM` : `${horaInicio%13+Math.trunc(horaInicio/13)} PM`;
    }
    ...
    const getHoraFinTexto = (horaInicio) => {
        ...
        return horaFin < 12 ? `${horaFin} AM` : `${horaFin%13+Math.trunc(horaFin/13)} PM`;
    }
    ...
    useEffect(() => {
        if (existsInSessionStorage("fecha") &&
            ...
            setHoraInicioTexto(getHoraInicioTexto(getFromSessionStorage("horaInicio")));
            setHoraFinTexto(getHoraFinTexto(getFromSessionStorage("horaInicio")));
        } else {
            setShowText(false);
        }
    }, [update]);
    ...
    return (
        ...
        {showText ? 
            <div className="p-5 text-white flex flex-col items-center">
                <p>{diaTexto} - {diaNumero} de {mes}</p>
                <p className="text-lg font-bold" >{horaInicioTexto} - {horaFinTexto}</p>
            </div>
         : 
            <></>
        }
